...
int heightSection = 36;//圆台高度细分数
int laySection = 72;//圆台每层扇面细分数
float radiu1 = 200;//圆台上底面半径
float radiu2 = 100;//圆台下底面半径
float layerHeight = 20;//每层高度

float rotateRange = PI;//圆台轴向扭曲弧度值
...
void setSystem() {
  initiateModelData();//初始化构成圆台的空间向量数组
...
void draw() {
  ...
  slider.draw();
  cam.endHUD();
  if ((mouseX<500 && mouseY< 400) || (mouseX<150 && mouseY > height-150)) {//此限定区域为滑块所在位置
    cam.setActive(false);
  } else {
    cam.setActive(true);
  }
...
void initiateModelData() {
  ...
  for (int i = 0; i < vp.length; i ++) {
    ...
    float sectionPhaseAngle = rotateRange / vp.length;//圆台整体扭曲角分配到每层的相位值
    float radiu = map(i, 0, vp.length-1, radiu1, radiu2);//每一层的半径随层数线性变化
